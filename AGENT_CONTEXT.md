...
**Default stance — worktree first; don't ask to edit main.** Unless Gabriel raised it *first in this session* ("do this on main" / "edit main directly" / "land it on main now"), the default for all work is a worktree. Do **not** ask Gabriel for permission to edit `main` — working on main is not a choice you put to him, it's one he initiates. If you're in a plain `main` session and need to make edits, the move is to relocate the work into a worktree (`node spawn-worktree.js <name>` and edit the worktree copy, or commit in the harness worktree and hand off a cherry-pick) — **not** to request approval for direct-to-main edits. The guardrail prompt below exists to catch the case where you defaulted wrong; when it fires, back out into a worktree rather than asking Gabriel to approve. The lone exception is a change Gabriel has, this session, explicitly told you to put on main now.
...
**Agents always work in a worktree, never directly in `main`.** This is NOT automatic for the CLI. A `claude` session's cwd is whatever directory it was launched in — so a plain `claude` started in the project root runs *in main*, and edits hit main directly. A session is only worktree-isolated if it was launched one of these ways:

- `claude --worktree <name>` (or `-w`) — starts the session in `.claude/worktrees/<name>/` on branch `worktree-<name>`. This is the supported per-session way to isolate the **interactive** session. (Earlier versions of this doc claimed the harness auto-places every session in a worktree — that's false for the CLI. Auto-worktrees only happen via `--worktree`, via a subagent with `isolation: worktree`, or in the desktop app.)
- launched from a `neos-city-worktrees/<name>/` directory created by `node spawn-worktree.js <name>` (the parallel-agent flow below).

In an isolated session, edits go in the worktree; commit on the worktree branch and hand off a `git cherry-pick <branch-or-sha>` command for Gabriel to land it on main. **If your cwd is main** (plain `claude`), do NOT silently edit — the guardrail below will prompt Gabriel on every main-targeting `Write`/`Edit`/`NotebookEdit`. Treat that prompt as a signal that you've defaulted wrong: move the work into a worktree. The only time the right response is to approve the prompt is when Gabriel already asked, this session, for the change to land on main now — and that should be rare. Don't surface the prompt as a way to ask him "main or worktree?"; default to worktree and act.

**Guardrail: editing `main` requires approval (enforced).** A `PreToolUse` hook (`.claude/hooks/guard-main-edits.js`, wired in `.claude/settings.json`) inspects every `Write`/`Edit`/`NotebookEdit`. If the target file lives in the main checkout (`C:\Users\pitag\Documents\neos-city\...`, excluding the `.claude/worktrees/` subtree) it returns `permissionDecision: "ask"`, so Gabriel gets a prompt for that specific edit. For any non-main path the hook is **ask-only** — it emits nothing and hands control back to the normal permission flow; it does NOT auto-approve. So a worktree edit is silent only when a permission rule already covers it: `.claude/settings.json` now allowlists `Edit`/`Write` for `neos-city-worktrees/**` (the harness `.claude/worktrees/<slug>/` subtree is silent under `acceptEdits` mode). Two separate gotchas to keep straight: (1) the hook's main-vs-worktree test is a path-prefix check, and `neos-city-worktrees/` is a *sibling* of the main checkout (not under `neos-city/`), so it is correctly treated as not-main; (2) the search tools `Grep`/`Glob` are confined to the session's working roots, and that same sibling worktree is *outside* those roots — so searching it fails with "Path is outside allowed working directories". That is a workspace-boundary check, NOT a permission rule, so no `Grep(...)` allowlist entry fixes it: either register the worktree via `permissions.additionalDirectories` (or `/add-dir`), or just Grep the identical file in the in-scope main checkout and Edit the worktree copy. `Read`/`Edit`/`Write` reach worktree paths through the permission layer and are unaffected by that boundary. The standing rule for agents: **work in a worktree by default; edit `main` directly only when Gabriel has, this session, raised it first and asked for the change to land on main now.** A main edit is never something you propose — it's something Gabriel initiates. When a main session needs to do real work, the supported move is to spin up a worktree (`node spawn-worktree.js <name>`) and edit the worktree copy, or commit in the harness worktree and hand off a cherry-pick — not to approve a pile of direct-to-main prompts. The hook is detection-and-prompt only; it can't relocate an in-progress session, so the *intent* still lives here in the doc. Don't weaken or remove the hook without Gabriel's say-so.
...
**When organizer profiles are 403-blocked from Node** (Challonge bot detection — now blocks all 7 organizers, not just `rigz_`), use `harvest_console.js`. Run `node prep_console.js harvest_console.js` to inline `ADMIN_TOKEN` and copy to clipboard, then paste into DevTools on **any** `challonge.com` page (the script walks all 7 organizer profiles via same-origin `fetch()` in one paste — no per-org navigation needed). It POSTs to `/api/tournaments/append-harvest`, which dedupes against `harvested_tournaments.txt` + DB, runs `validatePokkenSlugs()` to drop non-Pokkén entries, and appends new validated URLs. `https://challonge.com` is in the CORS allow list (`backend/src/app.js`). `pull_new.js` Step 2 offers this prompt automatically after `harvest_new.js`.
...
- **Static syntax checks — keep them simple.** Plain JS: `node -c <file>`. JSX:
  ...
  esbuild dumps the transformed JS to stdout on success — the redirect suppresses that pollution. On valid JSX you get empty stdout + empty stderr + exit 0; on invalid JSX, the parse error goes to stderr and surfaces in the tool result. Whitelisted for both shells in `.claude/settings.json`. Common mistakes:
  ...
  - **Redirecting to inspect** (`> /tmp/out; tail | head`, `| Out-File`) — the tool result already has stdout+stderr. Plus `/tmp/` is outside cwd, so the write itself prompts. (Redirecting to discard via `> /dev/null` or `> $null` is fine — different intent.)
  - **`| head -N` / `| Select-Object -First N`** — unnecessary. Both tools auto-truncate output at ~30k chars and tell you they did. A hardcoded `head -30` cap is noisier (extra pipe) and worse than the default: esbuild error spew can run 20+ lines per error, and stacked errors would get clipped at an arbitrary line.
  - **`> nul` in Bash** — `nul` is a literal filename in Git Bash, not the null sink. Use `> /dev/null`.
  - **`2>&1`** — different problem in each shell:
    - Bash: harmless to `$?`, but redundant noise. The tool result already returns stdout and stderr labelled separately; merging them just interleaves esbuild errors with the transformed JS on stdout.
