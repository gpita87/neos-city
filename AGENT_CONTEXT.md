...
**Agents always work in a worktree, never directly in `main`.** Claude Code auto-places you in an ephemeral harness worktree at `.claude/worktrees/<slug>/` — that's your cwd. Edits go in the worktree; main is read-only from here (see the "Read main, never edit it" rule below). Commit on the worktree branch and hand off a `git cherry-pick <branch-or-sha>` command for Gabriel to land it on main. If you find yourself running with cwd = main, flag that to Gabriel rather than silently editing — the worktree isolation is what prevents the Windows-mount truncation issue described in the next paragraph.
